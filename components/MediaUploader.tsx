...
export interface MediaUploaderProps {
  onUploadSuccess: (result: R2UploadResult, file: File) => void | Promise<void>;
...
export default function MediaUploader({
  ...
  const startUpload = useCallback(
    async (file: File) => {
      ...
      try {
        const result = await uploadMediaToR2(file, setProgress);
        setPublicUrl(result.publicUrl);
        await onUploadSuccess(result, file);
        setState("success");
      } catch (err) {
        const message =
          ...
          {state === "success" && selectedFile && publicUrl && (
            <>
              ...
              <button
                ...
                className="pointer-events-auto mt-5 rounded-md border border-white/10 bg-[#121217] px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-300 transition-colors hover:border-[#d4af37]/40 hover:text-white"
              >
                Upload another
              </button>
            </>
