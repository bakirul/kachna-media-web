...
import { fetchMediaAssets } from "@/utils/mediaAssets";
  ...
  useEffect(() => {
    ...
    async function loadCloudFolders() {
      try {
        ...
        const assets = await fetchMediaAssets({ userId: user.id });
        const folders = new Set<string>();

        assets.forEach((asset) => {
          if (asset.folder) {
            folders.add(asset.folder.replace(/\/+$/, ""));
          }
        });

        if (!cancelled) {
          setCloudFolders(Array.from(folders));
        }
      } catch (error) {
        console.error("Failed to load cloud folder tree:", error);
      }
    }
