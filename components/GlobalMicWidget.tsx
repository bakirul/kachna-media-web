...
export default function GlobalMicWidget({ socket, user }: GlobalMicWidgetProps) {
  ...
  return (
    <div className="mt-3 w-full pointer-events-auto flex items-center gap-2 font-sans">
      <button 
        onClick={() => setIsMicActive(!isMicActive)}
        className={`w-full px-5 py-2.5 rounded-full text-[11px] font-bold tracking-wide transition-all duration-300 backdrop-blur-md border shadow-2xl flex items-center justify-center gap-3 ${
          isMicActive 
      ...
      >
        ...
        <span className="truncate">{isMicActive ? "Live Translation Active" : "Enable Live Mic"}</span>
      </button>
    </div>
  );
}
