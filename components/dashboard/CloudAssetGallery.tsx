...
import { getMediaPlaybackUrl, isMediaAssetVideo } from "@/utils/mediaAssets";
...
export default function CloudAssetGallery({
  ...
  const { previewFile, selectGalleryRange, toggleGalleryAssetSelection, setGallerySelectionAnchorId } =
    useDashboardStore();
  const {
    aspectClass,
    objectFitClass,
  ...
  const renderPlayOverlay = (asset: MediaAssetRecord, compact = false) => (
    <button
      type="button"
      data-no-marquee
      onClick={(e) => handleVideoPreview(e, asset)}
      ...
      {filtered.map((asset) => {
                ...
                isVideo
                  ? (e) =>
                      handleCardPointerSelect(e, selectable, () =>
                        handleVideoPreview(e, asset),
                      )
                  : (e) => handleCardPointerSelect(e, selectable)
              }
            >
              ...
              <div className="relative flex h-10 w-16 shrink-0 items-center justify-center overflow-hidden rounded bg-[#0a0a0f]">
                {isImage ? (
                  ...
                  <>
                    <video
                      src={`${playbackUrl}#t=0.5`}
                      className={`h-full w-full ${objectFitClass}`}
                      preload="metadata"
                      muted
                    />
                    {renderPlayOverlay(asset, true)}
                  </>
                ) : (
              ...
              <AssetContextMenu asset={selectable} shareUrl={playbackUrl} />
            </div>
          );
        }
        ...
        return (
          <div
            ...
            data-gallery-selectable
            data-gallery-asset-id={asset.id}
            className={`group overflow-hidden rounded-lg border bg-[#121217] transition-colors ${
              isSelected
                ? "border-[#d4af37]"
